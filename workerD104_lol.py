#conda activate cellpose2&&python D:\Carlos\Scripts\workerDECONVOLUTION_BBD104compNewDrift.py
#
######### Please add documentation
### This is experiemnt D104, we are analyzing the big tad to supplement the samll tad. Date started: 9/6/23 by Carlos
...
save_folder = r'/mnt/merfish8v2/20230801D104Myh67d80/DNA_singleCy5/AnalysisDeconvolveCG2'
#/home/cfg001/Desktop/D104-main/flat_field/
...
def compute_drift_features(save_folder,fov,all_flds,set_,redo=False,gpu=False):
    fls = [fld+os.sep+fov for fld in all_flds]
    for fl in fls:
        #try:
...
def main_f(set_ifov,try_mode=True):
   ...
   # r'\\merfish6\merfish6v2\DNA_FISH\20230212_R120PVTS32RDNABDNF\'
    ...
    #all_flds_ = glob.glob(r'/mnt/merfish8v2/20230801D104Myh67d80/DNA_singleCy5/H*R*'+set__) ################################# modify for single cy5 small tad 
    #redefine_color += [[1,3]]*len(all_flds_) #### single signal color
    #all_flds+=all_flds_
    
    #T:\20230801D104Myh67d80DNA\DNA\ (this is for the big tad, has two sections)
    all_flds_ = glob.glob(r'/mnt/merfish8v2/20230801D104Myh67d80DNA/DNA/H*B*'+set__) ################################# modify for 3-col small tad 
    redefine_color += [[0,1,2,3]]*len(all_flds_) #### three signal color
    all_flds+=all_flds_

    #X:\20230801D104Myh67d80\DNA (remaining big TAD)
    all_flds_ = glob.glob(r'/mnt/merfish8v1/20230801D104Myh67d80/DNA/H*B*'+set__) ################################# modify for 3-col small tad 
    redefine_color += [[0,1,2,3]]*len(all_flds_) #### three signal color
    all_flds+=all_flds_
    #\\merfish8\merfish8v1\20230801D104Myh67d80\RNA introns
    all_flds_ = glob.glob(r'/merfish8/merfish8v1/20230801D104Myh67d80/DNA/H*I*'+set__) ################################# modify for 3-col small tad 
    redefine_color += [[0,1,2,3]]*len(all_flds_) #### th
    all_flds+=all_flds_
    '''
